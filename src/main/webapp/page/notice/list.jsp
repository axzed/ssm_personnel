...
<%--      <button type="button" onclick="location.href='${ctx}/document/add'" class="layui-btn layui-btn-small" style="line-height:1.6em;margin-top:3px;float:innert;margin-left:75%;;"  ><i class="layui-icon"></i>增加</button>--%>
      <a class="layui-btn layui-btn-small" style="line-height:1.6em;margin-top:3px;float:right" href="${ctx }/notice/pageByCondition" title="刷新">
        <i class="layui-icon" style="line-height:30px">ဂ</i></a>
    </div>
    <div class="x-body">
      <div class="layui-row" style="" align="center">
        <form class="layui-form layui-col-md12 x-so" method="get" action="${ctx }/notice/pageByCondition">
          <input type="text" name="title" style="width:50%;"  placeholder="请输入查找标题" autocomplete="off" class="layui-input">
        ...
        <c:choose>
            <c:when test="${sessionScope.tip  == 1 }">
                <xblock>
                    ...
                    <button class="layui-btn" onclick="x_admin_show('添加公告','${ctx}/notice/add')"><i class="layui-icon"></i>添加</button>
                    <span class="x-right" style="line-height:40px">共有公告 ${pageInfo.total} 条</span>
                </xblock>
            </c:when>
            <c:otherwise>
                <xblock>
     ...
     <tr>
            <td>
              <div class="layui-unselect layui-form-checkbox" lay-skin="primary" data-id='${notice.id}'><i class="layui-icon">&#xe605;</i></div>
            </td>
            <td>${status.count}</td>
            <td>${notice.title }</td>
            <td>${notice.content }</td>
            <td><fmt:formatDate pattern="yyyy-MM-dd HH:mm:ss" value="${notice.createDate }"></fmt:formatDate></td>
            <td>${notice.adminInfo.nickname }</td>
